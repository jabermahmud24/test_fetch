...
class MoveBaseClientNS:
    def __init__(self, ns="/locobot", wait_sec=8.0):
        ...
        if not ok:
            rospy.logwarn(f"[LoCoBot] No action server at {self.name}. "
                          f"Start your navigation stack (amcl/map_server/move_base) "
                          f"**in the same namespace** or set ns='' if it runs globally.")
            raise rospy.ROSException(f"move_base not available at {self.name}")

    def goto(self, x, y, yaw, frame="map", wait=True):
        g = MoveBaseGoal()
...
class LocobotCommander:
    ...
    def __init__(self, mb_ns="/locobot"):
        ...
        self.nav = None
        try:
            self.nav = MoveBaseClientNS(ns=mb_ns, wait_sec=8.0)
            rospy.loginfo(f"[LoCoBot] Connected to {self.nav.name}")
        except rospy.ROSException:
            rospy.logwarn("[LoCoBot] Navigation not available yet; "
                          "you can still use cmd_vel methods.")
            
    def goto(self, x, y, yaw, frame="map"):
        if not self.nav:
            raise RuntimeError("LoCoBot move_base is not running. "
                               "Launch your nav stack (map+amcl+move_base) in /locobot "
                               "or instantiate with mb_ns='' if move_base is global.")
        self.nav.goto(x, y, yaw, frame=frame, wait=True)
...
class LoCoDemo:
    def __init__(self):
        self.bot = LocobotCommander(mb_ns="/locobot")

    # def run(self):
    #     # Example targets (edit as needed)
    #     # Use frame="map" if you have map+AMCL; otherwise try frame="odom".
    #     rospy.loginfo("[LoCoBot] Navigating to waypoint A …")
    #     self.bot.goto(2.0, 1.0, 0.0, frame="map")

    #     rospy.loginfo("[LoCoBot] Navigating to waypoint B …")
    #     self.bot.goto(4.6, -6.0, 0.0, frame="map")

    #     rospy.loginfo("[LoCoBot] Reached B.")
