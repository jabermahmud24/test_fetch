...
# # Number of fixed human-guided midpoints\ n_guided = 3
# human_guided = torch.tensor([
#     [
#         [2.2, 0.6, 0.25, math.pi/2, math.pi, 0],  # FIX: Changed yaw for better orientation
#         [2.1, 0.7, 0.30, math.pi/2, math.pi, 0],
...
# # Door and workspace definitions\ nDOOR_X_MIN, DOOR_X_MAX = 1.4, 1.43
# DOOR_Y_OPEN_MIN, DOOR_Y_OPEN_MAX = 0.9, 1.2
# DOOR_Z_MIN, DOOR_Z_MAX = 0.0, 1.2
# WS = {"x": (0.0, 3.0), "y": (0.0, 2.0), "z": (0.0, 2.0)}
...
# Signed-distance and ESDF penalties
...
def collision_cost(vertices: torch.Tensor, clearance: float=0.01) -> torch.Tensor:
    cost = 0.0
    for w in WALLS:
        cost += esdf_penalty(vertices, w, clearance=clearance).sum(-1)
...
# Collision memory class as before
...
# -----------------------------------------
# Updated random_scene with env_feat
# -----------------------------------------
def random_scene(batch_size: int):
    ...
    env_vec = torch.tensor([
        DOOR_X_MIN, DOOR_X_MAX,
        DOOR_Y_OPEN_MIN, DOOR_Y_OPEN_MAX,
        DOOR_Z_MIN,   DOOR_Z_MAX
    ],dtype=torch.float32,device=DEVICE)
    return start_pose.repeat(batch_size,1), \
           goal_pose.repeat(batch_size,1), \
           env_vec.repeat(batch_size,1)

# -----------------------------------------
# WaypointNet with environment input
# -----------------------------------------
class WaypointNet(nn.Module):
    def __init__(self,n_mid=N_MID,env_dim=6):
        super().__init__()
        self.n_mid=n_mid
        input_dim=6+6+env_dim
        ...
        with torch.no_grad():
            ...
            for i in range(n_mid):
                x,y,z=1.9,1.1,0.5
                bias+=[x,y,z]
                human_rpy=human_guided[0,-1,3:].cpu().numpy()
                bias+=human_rpy.tolist()
            self.fc[-1].bias.copy_(torch.tensor(bias,dtype=torch.float32))
            nn.init.zeros_(self.fc[-1].weight)
    def forward(self,start_pose,goal_pose,env_feat):
        x=torch.cat([start_pose,goal_pose,env_feat],dim=-1)
        out=self.fc(x)
...
def train():
    global opt_upper
    net=WaypointNet(env_dim=6).to(DEVICE)
    ...
    for epoch in range(1,N_EPOCHS+1):
        start_pose,goal_pose,env_feat=random_scene(BATCH_SIZE)
        mid_init=net(start_pose,goal_pose,env_feat)
        ...
        print(f"Epoch {epoch}/{N_EPOCHS} | Upper Loss {loss_upper.item():.4f} | Lower Loss {loss_lower_final.item():.4f}")
